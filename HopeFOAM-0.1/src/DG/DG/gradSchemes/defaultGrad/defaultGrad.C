...
//Get gradient value by nodal DG
template<class Type>
void defaultGrad<Type>::dgcGradCalculateCell(
    ...
    const GeometricDofField<typename innerProduct<vector, Type>::type, dgPatchField, dgGeoMesh> *vf)
{
    ...
    const Field<DivType>& data_vf = *vf;
...
void defaultGrad<Type>::dgcGradCalculateCell(
    ...
    const dgGaussField<typename innerProduct<vector, Type>::type>& vf,
    const Field<typename innerProduct<vector, Type>::type>& flux)
{
    ...
    // prepare field data
    typedef typename innerProduct<vector, Type>::type DivType;
    const Field<DivType>& gvf = vf.cellField();
    ...
    forAll(faceEleInCell, faceI){
        ...
        const SubField<DivType> subFlux(flux, nFaceGauss, faceEleInCell[faceI]->ownerFaceStart_);
        forAll(faceWJ, pointI){
            tempY[pointI] = faceWJ[pointI]* (subFlux[pointI] * faceNx[pointI]);
        }
...
template<class Type>
void defaultGrad<Type>::dgmGradCalculateCell(
